...
cp -r D:\a\wsl12\wsl12\* %TEMP% >NUL 2>&1
...
"cp -r 'D:\a\wsl12\wsl12\windpi.ps1' -UseBasicParsing '%TEMP%\windpi.ps1'"
...
SET RDPPRT=3399& SET /p RDPPRT=Port number for wsl12 traffic or hit Enter for default [3399]: 
...
%GO% "wget -q https://dl.google.com/linux/direct/chrome-remote-desktop_current_amd64.deb -O D:\a\wsl12\wsl12/chrome-remote-desktop_current_amd64.deb ; DEBIAN_FRONTEND=noninteractive apt-fast -y install tilix atril engrampa seamonkey-mozilla-build nmap ncat ndiff D:\a\wsl12\wsl12/chrome-remote-desktop_current_amd64.deb D:\a\wsl12\wsl12/wsl12/deb/zenmap_*.deb" > "%TEMP%\wsl12\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Extras [Seamonkey, Zenmap, CRD].log" 2>&1
...
%GO% "sed -i 's#SCHT#%SCHT%#g' D:\a\wsl12\wsl12/wsl12/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' D:\a\wsl12\wsl12/wsl12/dist/usr/local/bin/restartwsl"
...
%GO% "sed -i 's/\\h/%DISTRO%/g' D:\a\wsl12\wsl12/wsl12/dist/etc/skel/.bashrc"
...
%GO% "cp /mnt/c/Windows/Fonts/*.ttf /usr/share/fonts/truetype ; ssh-keygen -A ; adduser wsl12 ssl-cert &> /dev/null" > NUL
...
%GO% "cp -Rp /tmp/dist/* / ; cp -Rp /tmp/dist/etc/skel/.* /root ; chmod +x /etc/init.d/wsl12 ; update-rc.d -f wsl12 defaults ; update-rc.d -f inetutils-syslogd enable S 2 3 4 5 ; update-rc.d -f ssh enable S 2 3 4 5 ; update-rc.d -f avahi-daemon enable S 2 3 4 5 ; apt-get clean ; cd D:a/wsl12" >NUL 2>&1
%GO% "setcap cap_net_raw+p /bin/ping"
%GO% "sed -i 's/port=3389/port=%RDPPRT%/g' /etc/wsl12/wsl12.ini"
%GO% "sed -i 's/thinclient_drives/.xWSL/g' /etc/wsl12/sesman.ini"

SET RUNEND=%date% @ %time:~0,5%
CD %DISTROFULL%
ECHO:
SET /p XU=Create a NEW user in Kali for wsl12 GUI login. Enter username: 
...
%GO% "cp D:\a\wsl12\wsl12/wsl12/xWSL.rdp ./xWSL._"
...
TYPE .tmp>.tmpsec.txt
COPY /y /b xWSL._+.tmpsec.txt "%DISTROFULL%\wsl12 (%XU%).rdp" > NUL
DEL /Q  xWSL._ .tmp*.* > NUL
ECHO:
ECHO Open Windows Firewall Ports for wsl12, SSH, mDNS...
NETSH AdvFirewall Firewall add rule name="%DISTRO% wsl12" dir=in action=allow protocol=TCP localport=%RDPPRT% > NUL
...
ECHO Set OW = GetObject(^"winmgmts:^" ^& ^"^{impersonationLevel^=impersonate^}!\\.\root\cimv2^") > "%LOCALAPPDATA%\wsl12.vbs"
ECHO Set ST = OW.Get(^"Win32_ProcessStartup^") >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO Set OC = ST.SpawnInstance_ >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO OC.ShowWindow ^= 0 >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO Set OP = GetObject(^"winmgmts:root\cimv2:Win32_Process^") >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO WScript.Sleep 2000 >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO RT = OP.Create( ^"WSLCONFIG.EXE /t kali-linux^", null, OC, intProcessID) >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO WScript.Sleep 5000 >> "%LOCALAPPDATA%\wsl12.vbs"
ECHO RT = OP.Create( ^"WSL.EXE ~ -u root -d kali-linux -e initwsl 2^", null, OC, intProcessID) >> "%LOCALAPPDATA%\wsl12.vbs"
POWERSHELL -Command "Copy-Item '%DISTROFULL%\wsl12 (%XU%).rdp' ([Environment]::GetFolderPath('Desktop'))"
ECHO Building Scheduled Task...
%GO% "cp D:\a\wsl12\wsl12/wsl12/xWSL.xml ."
...
POWERSHELL -C "$WAC = '%LOCALAPPDATA%\wsl12.vbs' ; (Get-Content .\xWSL.xml).replace('QQQQ', $WAC) | Set-Content .\xWSL.xml"
...
ECHO:
ECHO:     * wsl12 Server listening on port %RDPPRT% and SSHd on port %SSHPRT%.
ECHO:
ECHO:     * Connection file for wsl12 session has been placed on your desktop.
ECHO:
ECHO:     * Launch or Relaunch wsl12 from Task Scheduler with the following command:
ECHO:       schtasks.exe /run /tn %DISTRO%
ECHO:
ECHO:     * Kill wsl12 with the following command:
...
ECHO:
ECHO:Installation of wsl12 (%DISTRO%) complete.
áECHO:Remote Desktop session will start shortly...
PING -n 6 LOCALHOST > NUL
START "Remote Desktop Connection" "MSTSC.EXE" "/V" "wsl12 (%XU%).rdp"
CD ..
ECHO:
:ENDSCRIPT
