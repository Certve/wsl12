...
PowerShell.exe -Command "wsl --set-default-version 2 --install -d kali-linux -e 'uname' > $env:TEMP\DistroTestAlive.TMP ; $alive = Get-Content $env:TEMP\DistroTestAlive.TMP ; IF ($Alive -ne 'Linux') { Start-BitsTransfer https://aka.ms/wsl-kali-linux-new -Destination $env:TEMP\Kali.AppX ; WSL.EXE --set-default-version 1 > $null ; Add-AppxPackage $env:TEMP\Kali.AppX ; Write-Host ; Write-Host 'NOTE: Open the "Kali Linux" app from your Start Menu.' ; Write-Host 'When Kali initialization completes' ; PAUSE ; Write-Host }"
...
"cp -r 'D:\a\xWSL\xWSL\windpi.ps1' -UseBasicParsing '%TEMP%\windpi.ps1'"
...
SET RDPPRT=3399& SET /p RDPPRT=Port number for xWSL traffic or hit Enter for default [3399]: 
...
%GO% "wget -q https://dl.google.com/linux/direct/chrome-remote-desktop_current_amd64.deb -O D:\a\xWSL\xWSL/chrome-remote-desktop_current_amd64.deb ; DEBIAN_FRONTEND=noninteractive apt-fast -y install tilix atril engrampa seamonkey-mozilla-build nmap ncat ndiff D:\a\xWSL\xWSL/chrome-remote-desktop_current_amd64.deb D:\a\xWSL\xWSL/xWSL/deb/zenmap_*.deb" > "%TEMP%\xWSL\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Extras [Seamonkey, Zenmap, CRD].log" 2>&1
...
%GO% "sed -i 's#SCHT#%SCHT%#g' D:\a\xWSL\xWSL/xWSL/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' D:\a\xWSL\xWSL/xWSL/dist/usr/local/bin/restartwsl"
...
%GO% "sed -i 's/\\h/%DISTRO%/g' D:\a\xWSL\xWSL/xWSL/dist/etc/skel/.bashrc"
...
%GO% "cp /mnt/c/Windows/Fonts/*.ttf /usr/share/fonts/truetype ; ssh-keygen -A ; adduser xWSL ssl-cert &> /dev/null" > NUL
...
%GO% "cp -Rp /tmp/dist/* / ; cp -Rp /tmp/dist/etc/skel/.* /root ; chmod +x /etc/init.d/xWSL ; update-rc.d -f xWSL defaults ; update-rc.d -f inetutils-syslogd enable S 2 3 4 5 ; update-rc.d -f ssh enable S 2 3 4 5 ; update-rc.d -f avahi-daemon enable S 2 3 4 5 ; apt-get clean ; cd D:a/xWSL" >NUL 2>&1
%GO% "setcap cap_net_raw+p /bin/ping"
%GO% "sed -i 's/port=3389/port=%RDPPRT%/g' /etc/xWSL/xWSL.ini"
%GO% "sed -i 's/thinclient_drives/.xWSL/g' /etc/xWSL/sesman.ini"

SET RUNEND=%date% @ %time:~0,5%
CD %DISTROFULL%
ECHO:
SET /p XU=Create a NEW user in Kali for xWSL GUI login. Enter username: 
...
%GO% "cp D:\a\xWSL\xWSL/xWSL/xWSL.rdp ./xWSL._"
...
TYPE .tmp>.tmpsec.txt
COPY /y /b xWSL._+.tmpsec.txt "%DISTROFULL%\xWSL (%XU%).rdp" > NUL
DEL /Q  xWSL._ .tmp*.* > NUL
ECHO:
ECHO Open Windows Firewall Ports for xWSL, SSH, mDNS...
NETSH AdvFirewall Firewall add rule name="%DISTRO% xWSL" dir=in action=allow protocol=TCP localport=%RDPPRT% > NUL
...
ECHO Set OW = GetObject(^"winmgmts:^" ^& ^"^{impersonationLevel^=impersonate^}!\\.\root\cimv2^") > "%LOCALAPPDATA%\xWSL.vbs"
ECHO Set ST = OW.Get(^"Win32_ProcessStartup^") >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO Set OC = ST.SpawnInstance_ >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO OC.ShowWindow ^= 0 >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO Set OP = GetObject(^"winmgmts:root\cimv2:Win32_Process^") >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO WScript.Sleep 2000 >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO RT = OP.Create( ^"WSLCONFIG.EXE /t kali-linux^", null, OC, intProcessID) >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO WScript.Sleep 5000 >> "%LOCALAPPDATA%\xWSL.vbs"
ECHO RT = OP.Create( ^"WSL.EXE ~ -u root -d kali-linux -e initwsl 2^", null, OC, intProcessID) >> "%LOCALAPPDATA%\xWSL.vbs"
POWERSHELL -Command "Copy-Item '%DISTROFULL%\xWSL (%XU%).rdp' ([Environment]::GetFolderPath('Desktop'))"
ECHO Building Scheduled Task...
%GO% "cp D:\a\xWSL\xWSL/xWSL/xWSL.xml ."
...
POWERSHELL -C "$WAC = '%LOCALAPPDATA%\xWSL.vbs' ; (Get-Content .\xWSL.xml).replace('QQQQ', $WAC) | Set-Content .\xWSL.xml"
...
ECHO:
ECHO:     * xWSL Server listening on port %RDPPRT% and SSHd on port %SSHPRT%.
ECHO:
ECHO:     * Connection file for xWSL session has been placed on your desktop.
ECHO:
ECHO:     * Launch or Relaunch xWSL from Task Scheduler with the following command:
ECHO:       schtasks.exe /run /tn %DISTRO%
ECHO:
ECHO:     * Kill xWSL with the following command:
...
ECHO:
ECHO:Installation of xWSL (%DISTRO%) complete.
áECHO:Remote Desktop session will start shortly...
PING -n 6 LOCALHOST > NUL
START "Remote Desktop Connection" "MSTSC.EXE" "/V" "xWSL (%XU%).rdp"
CD ..
ECHO:
:ENDSCRIPT
